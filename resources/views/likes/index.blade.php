                ...
                @foreach ($likedImages as $image)
                    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm rounded-xl max-w-md mx-auto">
                        ...
                        <div class="px-4 py-3 border-t border-gray-200 dark:border-gray-700 flex items-center gap-4">
                            ...
                            <!-- Comments List -->
                            <div id="comments-list-{{ $image->id }}" class="hidden comments-list space-y-2">
                                @foreach ($image->comments as $comment)
                                    <div class="bg-gray-100 dark:bg-gray-700 rounded-lg p-3 relative hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors">
                                        <div class="flex items-start justify-between">
                                            <div class="flex-1 min-w-0 mr-8">
                                                ...
                                                <p class="text-gray-700 dark:text-gray-300 text-sm truncate">{{ $comment->comment }}</p>
                                            </div>

                                            <!-- Delete Button -->
                                            @if (Auth::id() === $comment->user_id || Auth::id() === $image->user_id)
                                                <form action="{{ route('comments.delete', $comment->id) }}" method="POST" class="flex-shrink-0">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
